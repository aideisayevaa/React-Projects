...
function App() {
  ...
  useEffect(() => {
    async function getApi() {
      try {
        //asagidaki kodda london sozunu silib evezine search deyisenini daxil edirik ki, istifadecinin daxil etdiyi seherin melumatlari gosterilsin
        const response = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${search}&appid=${key}`); //response daxilinde axcios.get metodu cagirir, ne vaxt gelse o vaxt gosterir () daxiline ise hansi Api-ni isteyirkse onun linkini yaziriq
 ...
 console.log(search)
  ...
  return (
    <div className="App mb-3 pt-0 w-{250px}">
      {/* istifadecidn melumatlari input vasitesile alacagiq*/}
      <input onChange={(e) => { setSearch(e.target.value) /* burada evvelce console.log yazmisdiq indi setSearch olaraq deyisdirdik cunki inputdan gelen her deyeri setSearch ile deyisiklik edirik */ }}
        className='border-4 bg-slate-500 ' type="text" /> {/* input daxilindeki deyer her deyisende deyisiklik olmasini isteyirikse onChange yaziriq */}
